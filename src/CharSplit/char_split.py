    ...
    for n in range(3, len(word) - 2):
        ...
        for k in range(len(word) + 1, 2, -1):
            ...
            in_slice_prob.append(
                ngram_probs.infix.get(in_ngram, 1))  # Favor ngrams not occurring within words
        ...
        in_slice_prob = min(
            in_slice_prob)  # Lowest, punish splitting of good ingrams
...
def germanet_evaluation(print_errors: bool = False):
    ...
    for line in open('split_compounds_from_GermaNet13.0.txt', 'r').readlines()[2:]:
        ...
        if cases % 10000 == 0: print(' Accuracy (' + str(correct) + '/' + str(cases) + '): ',
                                     100 * correct / cases)
